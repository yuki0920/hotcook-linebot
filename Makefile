...
deploy:
	@gcloud run deploy hotcook-linebot \
		...
		--set-env-vars=LINE_CHANNEL_ACCESS_TOKEN=${PROD_LINE_CHANNEL_ACCESS_TOKEN} \
		--set-env-vars=LINE_CHANNEL_SECRET=${PROD_LINE_CHANNEL_SECRET} \
		--set-env-vars=OPENAI_API_KEY=${PROD_OPENAI_API_KEY} \
